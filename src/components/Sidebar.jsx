...
const Sidebar = () => {
  ...
  return (
    <>
      <div className="md:flex hidden flex-col w-[180px] py-10 px-5 bg-black">
        <img src={logo} alt="logo" className="w-full h-14 object-contain" />
        <NavLinks />
      </div>

      {/* Mobile sidebar */}
      <div className="absolute z-20 md:hidden block top-6 right-3">
        {!mobileMenuOpen ? (
          <HiOutlineMenu
      ...
      <div
        className={`${
          mobileMenuOpen
            ? "bg-black absolute left-0 right-0 bottom-0 top-0 z-10 smooth-transition"
            : "bg-none hidden smooth-transition"
        }`}
      ></div>
      <div
